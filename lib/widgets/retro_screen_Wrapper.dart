...
class _RetroScreenWrapperState extends State<RetroScreenWrapper> {
  ...
  void _handleBack() async {
    ...
    if (now.difference(_lastSoundPlayed).inMilliseconds >= 300) {
      ...
      await _audioPlayer.play(AssetSource('select.mp3'));
...
class _BackButtonState extends State<_BackButton>
  ...
  void _handleTap() async {
    await _controller.forward(); // wait for forward animation
    await _controller.reverse(); // wait for reverse animation
    widget.onTap(); // call back only after animation completes
  }
...
class DotFade extends StatefulWidget {
  ...
  const DotFade({Key? key, required this.offsetFactor, this.size = 6.0})
      : super(key: key);
...
class _DotFadeState extends State<DotFade>
    with SingleTickerProviderStateMixin {
  late AnimationController _controller;
